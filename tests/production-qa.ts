...
test.describe('ARASS Production QA & Interactions Suite', () => {
  test('MegaMenu should open, update previews on hover, and close with ESC', async ({ page }) => {
    ...
    // Verify links exist
    await expect(menuDialog.getByText('01')).toBeVisible();
    await expect(menuDialog.getByText('MISSION')).toBeVisible();
  ...
  test('Contact form handles inquiry transmission and delivers receipt', async ({ page }) => {
    ...
    // Submit
    const submitBtn = page.getByRole('button', { name: /SEND TRANSMISSION/i });
    await submitBtn.click();

    // Expect receipt confirmation
    await expect(page.getByText('TRANSMISSION RECEIVED')).toBeVisible({ timeout: 5000 });
