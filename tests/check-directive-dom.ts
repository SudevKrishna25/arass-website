...
(async () => {
  ...
  const info = await page.evaluate(() => {
    ...
    const horizSpacer = horizon?.parentElement?.classList.contains('pin-spacer') ? horizon.parentElement : horizon;
    const directSpacer = directive?.parentElement?.classList.contains('pin-spacer') ? directive.parentElement : directive;
    ...
    return {
      ...
      horizonTop: horizSpacer ? horizSpacer.getBoundingClientRect().top + window.scrollY : 0,
      directiveTop: directSpacer ? directSpacer.getBoundingClientRect().top + window.scrollY : 0,
      bodyHeight: document.body.scrollHeight,
    };
  });

  console.log('Directive Layout Info:', info);
